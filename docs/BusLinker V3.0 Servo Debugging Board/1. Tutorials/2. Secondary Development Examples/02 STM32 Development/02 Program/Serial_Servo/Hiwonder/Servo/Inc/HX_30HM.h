...
#define ACK	1	/**< 0-不应答, 1-应答 (0 -No response, 1 -response) */
...
/**
 * @brief 初始化舵机控制系统 (Initialize servo control system)
 * @details 初始化串口通信和必要的硬件资源
 *          (Initialize serial communication and necessary hardware resources)
 */
...
/**
 * @brief 执行寄存器动作 (Execute register action)
 * @param id 舵机ID (Servo ID)
 * @return 舵机状态信息 (Servo status information)
 */
 ...
 * @return 舵机状态信息 (Servo status information)
 */
ServoStatus_t servo_read_pos_offset(uint8_t id, int16_t *value);
 ...
 * @return 舵机状态信息 (Servo status information)
 */
ServoStatus_t servo_read_moving_status(uint8_t id, uint8_t *status);

extern PacketControllerTypeDef servo_packet;
