...
- IMU sensors in sim (available in MJCF):
  - `chest_imu_quat` (MuJoCo `framequat`) and `chest_imu_gyro` (MuJoCo `gyro`) exist in `assets/v2/wildrobot.xml` (or the selected variant MJCF).
...
class ServoConfig:
    ...
    joint_angle_at_servo_center_deg: float  # MuJoCo angle (deg) represented by conceptual servo center (500)
    ...
    def joint_target_rad_to_elect_unit(self, target_rad: float) -> int:
      ...
      center_rad = math.radians(self.joint_angle_at_servo_center_deg)
...
2) **Set runtime config basics**
   - In `runtime/configs/hardware_config.json`, set:
     - `bno085.i2c_address`
...
│  │  └─ hardware_config.json              # calibration template (servo IDs, offsets, motor_unit_direction)
│  ├─ scripts/
...
- `export_policy_bundle(checkpoint_path, config_path, output_dir, robot_config_path)`
  - Writes: `policy.onnx`, `policy_spec.json`, `mujoco_robot_config.json` snapshot, `checksums.json`, `hardware_config.json`
  - `hardware_config.json` is generated from `runtime/configs/hardware_config.json`
...
- `hardware_config.json` ← runtime JSON seeded from `runtime/configs/hardware_config.json` (hardware settings)
...
`hardware_config.json` contains only physical robot configuration. Policy paths
and contracts are selected by the bundle manifest, not by hardware config.
