...
**Files Modified:** `training/envs/wildrobot_env.py`
...
**Files Modified:** `training/envs/wildrobot_env.py`, DR config files
...
**Files Modified:** `training/envs/jax_env_fns.py`, `training/envs/jax_full_port.py`
...
**Files Created:**
- `training/amp/discriminator.py` (~300 lines) - Full JAX/Flax implementation
- `training/amp/ref_buffer.py` (~300 lines) - Buffer + synthetic generators
- `training/amp/amp_wrapper.py` (~200 lines) - Reward wrapper for Brax
- `training/amp/amp_features.py` (~220 lines) - **Feature extraction (NEW)**
- `training/core/__init__.py` (~30 lines) - **Module exports (NEW)**
- `training/core/transitions.py` (~110 lines) - **AMPTransition dataclass (NEW)**
- `training/core/ppo_building_blocks.py` (~380 lines) - **GAE, PPO loss, networks (NEW)**
- `training/core/amp_ppo_training.py` (~620 lines) - **Custom training loop (NEW)**
- `training/train.py` (~260 lines) - **CLI entry point (NEW)**
...
**Files Modified:**
- `training/train.py` - Added --enable-amp flag and wrapper integration
...
- `training/core/ppo_building_blocks.py` - Already uses Flax/Optax correctly
- `training/core/amp_ppo_training.py` - Already uses optax.apply_updates
...
**Files Modified:**
- `training/train_brax_ppo.py` - Removed invalid parameter, updated comments
- `training/brax_wrapper.py` - **Major rewrite** for Pure-JAX backend
...
**Files:** `training/envs/wildrobot_env.py`, `training/envs/jax_full_port.py`
...
**Files Created:**
- `training/core/experiment_tracking.py` (~500 lines) - Unified tracking module

**Files Modified:**
- `training/train.py` - Integrated ExperimentTracker with full metrics logging
- `training/train.py` - Added import for ExperimentTracker
...
**Usage:**
```python
from training.core.experiment_tracking import ExperimentTracker
...
**Summary:** 16 walking motions from AMASS KIT dataset retargeted via GMR IK. Total 87.07 seconds (4,350 frames) of motion data at 50 FPS. Merged dataset at `training/data/walking_motions_merged.pkl`.
...
**Summary:** JAX/Flax discriminator with 1024-512-256 architecture, WGAN-GP gradient penalty, and 29-dim AMP feature extraction. Files: `training/amp/discriminator.py`, `training/amp/amp_features.py`.
...
**Summary:** Hyperparameters configured in `training/configs/wildrobot_phase3_training.yaml`. Key settings: num_envs=1024, rollout_steps=20, lr=3e-4, amp_weight=1.0.
...
**Files Created:**
- `training/core/trainer_jit.py` (~900 lines) - Fully JIT-compiled trainer

**Files Modified:**
- `training/train.py` - Added JIT trainer integration, `--legacy` flag for old trainer
...
python training/train_amp.py \
    --iterations 3000 \
    --num-envs 1024 \
    --amp-data training/data/walking_motions_merged.pkl
```
