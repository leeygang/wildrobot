...
uv run python training/data/gmr_to_physics_ref_data.py --verbose 2>&1 | grep "saturated"
```
...
2. Run micro-train smoke test:
   ...
   uv run python scripts/micro_train_smoke_test.py \
       --dataset training/data/tier0plus/tier0plus_merged.pkl \
       --steps 5000 \
       --log-interval 100
   ```
...
uv run python scripts/analyze_tier0plus_dataset.py \
    --json-output training/data/tier0plus/analysis.json
```
