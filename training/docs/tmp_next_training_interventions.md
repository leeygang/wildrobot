...
>
> **smoke12 (widen per-joint residual to cover prior amplitude)**:
> lands in this commit as
> `training/configs/ppo_walking_v0201_smoke12.yaml`.  Per-joint
> scales are generated by
> `assets/derive_residual_scales.py --coverage 0.75` against the
> current prior + home keyframe (knee_pitch ≈ 0.67, hip_pitch ≈
> 0.35-0.39, ankle_pitch ≈ 0.25 floor).  Re-run that script whenever
> the prior is regenerated; `--write CONFIG.yaml` splices the new
> block in place.  All other knobs inherit byte-equal from smoke11.
> Reference machinery still load-bearing in critic + rewards; actor
> obs unchanged.  Run smoke12 next; CHANGELOG.md will be updated
> once smoke12 results are confirmed (per AGENTS.md rule on
> training-result changelog entries).
