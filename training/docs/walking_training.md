...
- Pre-v0.20.1 milestone plan (v0.19.0 → v0.20.0-D), and per-smoke
  launch checklists for v0.20.1-smoke6-prep3 / smoke7 moved to
  [`archive/walking_training_v019_v0200_v0201_pre_smoke13_plan.md`](archive/walking_training_v019_v0200_v0201_pre_smoke13_plan.md)
  on 2026-05-24.
...
smoke8:              q_target = home_q + clip(action,±1)·scale_1.1 + RSI + stronger signed-vy training
smoke9 (planned):    q_target = home_q + clip(action,±1)·scale_1.1 + RSI + exact signed-vy curriculum
```
...
> `training/CHANGELOG.md` `[v0.21.0-smoke8-result + smoke9-plan]`. smoke6 now
...
> honestly but learned a one-sided lateral mode. smoke8 increased the lateral
> command range and `alpha_y`, but the TB-style ellipse still projected most
> train-side `vy` commands down to about `0.025 m/s`, so the policy kept the
> same positive-lateral mode under both `+vy` and `-vy` probes. smoke9 is
> therefore **not** a contract reset either — it is an exact signed-`vy`
> curriculum on the validated `home + RSI` path, plus an instrumentation fix for
...
The verbose v0.19.x → v0.20.0 history was archived 2026-05-24.
...
- **Prior-reference pivot complete** (v0.20.0-A → v0.20.0-D): ZMP
  is the executable prior, IK + short-horizon feasibility
  validated, PPO unblock decision taken.
- **v0.20.1 ToddlerBot-style PPO active** (smoke1 → smoke15).
  Smoke12b broke the standing basin and visually walked but with
  lateral drift; smoke13 isolated TB-style cmd-conditioned
  reference + 2D velocity tracking (failed: height-low collapse);
  smoke14 (the current line) added critic obs x15 stacking + 0.26
  m/s WR-scaled cmd range — height-low fixed but train→eval gap
  exposed a standing local minimum; smoke15 fixed the eval
  authority + deploy-facing eval metrics (lateral / world-drift /
  yaw with per-episode-abs aggregation).
...
- **Smoke8 result (2026-06-06): stronger `vy` range and `alpha_y` were not
  enough; no checkpoint promoted.** The authoritative post-training eval
  promoted **no** checkpoint. The top candidates still selected a positive
  lateral mode under both lateral probe signs:
  - rank-1 `checkpoint_2060_42188800.pkl`: straight eval
    `forward_velocity 0.187`, `cmd_err 0.0826` (fail),
    `step_length 0.119`, `lateral_velocity_abs 0.193` (fail),
    `world_y_drift_abs_m 0.646` (fail); probe `+0.065` achieved
    `+0.0754` (pass), probe `-0.065` achieved `+0.0741` (fail)
  - rank-2 `checkpoint_2560_52428800.pkl`: straight eval has the cleanest
    path drift (`world_y_drift_abs_m 0.143`) but still `lateral_velocity_abs
    0.177` and wrong-sign `-vy` probe (`+0.0396`, fail)

  Root cause: smoke8 widened `min/max_velocity_y` to `±0.065`, but kept the
  TB-style walk ellipse (`vy = uniform(deadzone_y, bound) * cos(theta)`).
  Train-side `tracking/velocity_cmd_vy_abs` therefore rose only from smoke7's
  `≈0.015` to smoke8's `≈0.025`, while the authoritative probes test
  `|vy|=0.065`. The reward separation from `alpha_y=120` is now adequate; the
  problem is insufficient exposure to exact signed lateral commands. Also,
  `reward/cmd_lateral_velocity_track` is emitted by the env but missing from
  `training/core/experiment_tracking.py::REWARD_TERM_KEYS`, so the lateral
  reward-factor diagnostic was absent from `metrics.jsonl`.

- **Smoke9 direction (2026-06-06): exact signed-`vy` curriculum before returning
  to the broader TB ellipse.** Resume again from the smoke6 60M promoted
  checkpoint, keep `home + RSI + scale_1.1`, `cmd_velocity_track_dim=2`,
  `alpha_x=562.5`, `alpha_y=120.0`, yaw off, and the same `±0.065` probes.
  Change only the command curriculum for this stage:
  - replace ellipse-projected lateral sampling with balanced exact signed bins,
    e.g. `[0.13,-0.065,0]`, `[0.13,0,0]`, `[0.13,+0.065,0]`
  - optionally allow a narrow forward band around `vx≈0.13`, but do not let
    lateral command magnitude collapse through `cos(theta)` during this stage
  - fix instrumentation before launch by adding
    `reward/cmd_lateral_velocity_track` to
    `training/core/experiment_tracking.py::REWARD_TERM_KEYS`

  This intentionally diverges from TB's broad ellipse sampler for one curriculum
  stage because WR has a confirmed one-sided lateral local minimum. Once both
  signs pass, return to the broader TB-style command coverage and then add yaw.

Per-run detail (verdicts, gates, checkpoint picks):
`training/CHANGELOG.md` for smoke13+,
`training/CHANGELOG.archive.md` for smoke12b and earlier.

### Pre-v0.20.1 milestones (archived)

The per-milestone plan subsections for v0.19.0, v0.19.1, v0.19.2,
v0.20.0 Prior Reference Pivot, and v0.20.0-A through v0.20.0-D
were moved to
`training/docs/archive/walking_training_v019_v0200_v0201_pre_smoke13_plan.md`
on 2026-05-24.  They describe completed work — the v0.20.1
ToddlerBot-style PPO framework below is the live mainline.
...
### Per-smoke milestone plans (archived)

The detailed launch checklists for `v0.20.1-smoke6-prep3` and
`v0.20.1-smoke7` were moved to
`training/docs/archive/walking_training_v019_v0200_v0201_pre_smoke13_plan.md`
on 2026-05-24.  Both have been superseded by the smoke13 / smoke14 /
smoke15 work documented in `training/CHANGELOG.md`; per-config launch
context now lives in the YAML headers under
...
| **DR** | **backlash** | **range [0.02, 0.10] rad, activation 0.1 Nm** ✓ FIXED (`yaml:env.domain_rand_backlash_range`) | activation 0.1, range [0.02, 0.1] rad (`mjx_config.py:209-210`) | ✅ Phase 3 landed.  TB-form smooth backlash: `motor_pos += 0.5 * backlash * tanh(qfrc_actuator / activation)` applied at obs time (mirror `mjx_env.py:2073-2080`). |
...
phases so smoke regressions are diagnosable.  All three phases plus
the bug-fix follow-up are landed.

**Phase 1 — yaml + docs (commit `6b66d73`):**
...
`alive` is now a dense per-step bonus (`+alive_w * dt` per surviving
step, mirror `_reward_alive` in `mjx_env.py:2766-2782`), not the
one-shot `-done` form WR previously used (which mirrored the
...
1. `ref_feet_z_track` — dense per-step foot-z error vs the ZMP
   prior's foot trajectory.  Weight **5.0**, α=1428.6 (= 1/0.0007
   mirroring TB `feet_phase_tracking_sigma`).  Substitute for TB
   `feet_phase` (`walk_env.py:631-695`) — WR uses the prior's
   actual foot-z at each step instead of TB's phase-derived
   expected height.  Direct addressee of v0.20.1-smoke1 shuffle
   exploit (step_length 0.022 m vs 0.030 m gate).
2. `penalty_pose` — per-joint weighted (q_actual - q_ref)^2 sum.
   Weight **-0.5** (negative-yaml convention; magnitude matches TB
   walk.gin:120 since TB uses positive scale × `_reward_penalty_pose`
   returning negative sum, `mjx_env.py:2707`).  Per-joint vector
   mirrors TB pattern: hip_pitch / knee_pitch=0.01, hip_roll=1.0,
   ankle_pitch / ankle_roll=5.0; arms / waist / wrist=0.
3. `penalty_feet_ori` — anti-tippy-toe via projected gravity in
   foot frame.  Weight **-5.0** (same sign convention as
   penalty_pose; magnitude matches TB walk.gin:129).  Mirror
   `walk_env.py:697-735`.
...
1. Asymmetric critic: privileged obs payload bumped from 12 dims
   ...
   `ppo.critic_privileged_enabled=true`.  Sample-efficiency win
   for locomotion PPO (Pinto et al. 2017 "Asymmetric Actor Critic"),
   especially valuable given the 50× compute gap.
2. Backlash domain randomization: TB-form smooth backlash —
   `motor_pos += 0.5 * backlash * tanh(qfrc_actuator / activation)`
   applied at obs time.  Per-joint sample range **[0.02, 0.10] rad**,
   activation **0.1 Nm** (mirror `mjx_config.py:209-210`,
   `mjx_env.py:2073-2080`).  Sim2real-critical noise model for
   hobby servos.

**Phase 3b — loader fix (bug-fix follow-up):**

The smoke yaml's Phase 2 / Phase 3 reward weights and DR ranges
were not being parsed by `training/configs/training_config.py` —
`_parse_reward_weights_config` and `_parse_env_config` stopped at
the pre-Phase-2 fields.  Without parser entries, the smoke would
load dataclass defaults (0.0 / empty) and silently disable every
Phase 2 / Phase 3 fix.  Loader now parses `ref_feet_z_track`,
`ref_feet_z_track_alpha`, `penalty_pose`, `penalty_feet_ori`,
`penalty_pose_weights_per_joint`, `penalty_pose_weight_default`,
`domain_rand_backlash_range`, and `domain_rand_backlash_activation`.

**Optional — kd / damping / armature / tau_max DR family** (deferred):
TB randomizes these in `mjx_config.py:222-235` ([0.8, 1.2] for
damping/armature, [0.9, 1.1] for kd / tau_max family).  WR doesn't
yet randomize them.  Low priority — backlash is the load-bearing
