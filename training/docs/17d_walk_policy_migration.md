...
- Physical wrist servos are outside the locomotion hardware configuration. A
  native 17D policy neither opens, reads, writes, nor synthesizes them.

The canonical `mujoco_robot_config.json`, policy specs, and hardware config all
contain the same 17 actuator names. There is no exclusion or externally-managed
configuration. `actuator_order.txt` is the single 17-joint order and each
keyframe contains the free root plus those 17 joint positions.
