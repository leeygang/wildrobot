...
def robot_schema():
    ...
    import json
    with open("training/schema/wildrobot_schema.json") as f:
        return json.load(f)
...
# By tier
uv run pytest training/tests -m "unit" -v          # T0: Pre-commit
uv run pytest training/tests -m "sim" -v           # T1: PR gate
uv run pytest training/tests -m "train" -v         # T1/T2: Training
uv run pytest training/tests -m "mjx" -v           # T2: MJX parity
uv run pytest training/tests -m "robust" -v        # T2/T3: Robustness

# By layer
uv run pytest training/tests/test_schema_contract.py -v    # Layer 0
uv run pytest training/tests/test_model_validation.py -v   # Layer 2
uv run pytest training/tests/test_state_mapping.py -v      # Layer 3
uv run pytest training/tests/test_reward_terms.py -v       # Layer 4
uv run pytest training/tests/test_env_step.py -v           # Layer 5
...
1. **Create schema directory**: `training/schema/wildrobot_schema.json`
