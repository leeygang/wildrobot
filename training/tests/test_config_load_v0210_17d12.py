...
BASE_CONFIG = Path(
    "training/configs/ppo_walking_v0210_17d11_native_stance_stage1.yaml"
)
CONFIG = Path("training/configs/ppo_walking_v0210_17d12_stance_width_retreat.yaml")
...
def test_17d12_changes_only_stance_width_and_metadata() -> None:
    assert _normalized_contract(CONFIG) == _normalized_contract(BASE_CONFIG)

    cfg = load_training_config(CONFIG)
    assert cfg.version == "0.21.0-17d12"
    assert cfg.env.loc_ref_default_stance_width_m == pytest.approx(0.0505)
