...
BASE_CONFIG = Path(
    "training/configs/ppo_walking_v0210_17d31_contact_free_teacher_distill.yaml"
)
CONFIG = Path(
...
def _normalized_contract(path: Path) -> dict:
    ...
    raw["bootstrap"]["commands"] = "<distillation-commands>"
    raw["checkpoints"].pop("dir")
    raw["wandb"].pop("tags")
    return raw


def test_17d32_changes_only_distillation_commands_and_metadata() -> None:
    ...
    assert cfg.version == "0.21.0-17d32"
    assert raw["bootstrap"]["commands"] == "0.065,0,0;0.13,0,0"
