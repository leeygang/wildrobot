...
CONFIG = Path("training/configs/ppo_walking_v0210_17d16_tilt_rate_finetune.yaml")
...
def _normalized_contract(path: Path) -> dict:
    ...
    raw["reward_weights"].pop("ang_vel_xy")
    raw["checkpoints"].pop("dir")
    raw["wandb"].pop("tags")
    return raw


def test_17d16_changes_only_tilt_rate_weight_and_metadata() -> None:
    assert _normalized_contract(CONFIG) == _normalized_contract(BASE_CONFIG)

    cfg = load_training_config(CONFIG)
    assert cfg.version == "0.21.0-17d16"
    assert cfg.reward_weights.ang_vel_xy == pytest.approx(1.78)
