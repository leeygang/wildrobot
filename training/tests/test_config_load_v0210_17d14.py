...
BASE_CONFIG = Path(
    "training/configs/ppo_walking_v0210_17d13_stance_width_retreat.yaml"
)
CONFIG = Path("training/configs/ppo_walking_v0210_17d14_stance_width_retreat.yaml")
...
def test_17d14_changes_only_stance_width_and_metadata() -> None:
    assert _normalized_contract(CONFIG) == _normalized_contract(BASE_CONFIG)

    cfg = load_training_config(CONFIG)
    assert cfg.version == "0.21.0-17d14"
    assert cfg.env.loc_ref_default_stance_width_m == pytest.approx(0.0525)
