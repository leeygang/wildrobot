from __future__ import annotations

import sys
from pathlib import Path

import jax
import jax.numpy as jnp
import pytest

from assets.robot_config import load_robot_config
from training.configs.training_config import load_training_config
from training.policy_spec_utils import build_policy_spec_from_training_config


_CFG = Path("training/configs/ppo_standing_stabilizer_v0230_narrow_stance.yaml")


def test_v0230_pins_short_low_lr_narrow_stance_diagnostic() -> None:
    cfg = load_training_config(str(_CFG))
    robot_cfg = load_robot_config(cfg.env.robot_config_path)
    spec = build_policy_spec_from_training_config(
        training_cfg=cfg,
        robot_cfg=robot_cfg,
    )

    assert cfg.version == "0.23.0"
    assert spec.observation.layout_id == "wr_obs_v9_standing"
    assert spec.model.obs_dim == 59
    assert spec.model.action_dim == 17
    assert cfg.ppo.iterations == 20
    assert cfg.ppo.learning_rate == pytest.approx(2.0e-6)
    assert cfg.ppo.log_interval == 1
    assert cfg.checkpoints.interval == 1
    assert cfg.reward_weights.feet_distance == 0.0
    assert cfg.reward_weights.penalty_narrow_stance == pytest.approx(10.0)
    assert cfg.reward_weights.penalty_close_feet_xy == pytest.approx(10.0)
    assert cfg.env.domain_rand_persistent_torso_pitch_error_range == pytest.approx(
        (-0.218166, 0.218166)
    )


def test_checkpoint_interval_defaults_to_config(monkeypatch: pytest.MonkeyPatch) -> None:
    from training.train import parse_args

    monkeypatch.setattr(sys, "argv", ["training/train.py"])
    args = parse_args()

    assert args.checkpoint_interval is None


def test_v0230_narrow_stance_penalty_matches_logged_distance() -> None:
    from training.core.metrics_registry import METRIC_INDEX, METRICS_VEC_KEY
    from training.envs.wildrobot_env import WildRobotEnv

    cfg = load_training_config(str(_CFG))
    load_robot_config(cfg.env.robot_config_path)
    env = WildRobotEnv(config=cfg)
    state = jax.jit(env.reset_for_eval)(jax.random.PRNGKey(0))
    state = jax.jit(env.step)(
        state,
        jnp.zeros(env.action_size, dtype=jnp.float32),
    )
    metrics = state.metrics[METRICS_VEC_KEY]
    distance = float(metrics[METRIC_INDEX["support/feet_lateral_distance_m"]])
    contribution = float(metrics[METRIC_INDEX["reward/penalty_narrow_stance"]])
    normalized_deficit = max(
        0.0,
        min(1.0, (cfg.env.min_feet_y_dist - distance) / cfg.env.min_feet_y_dist),
    )
    expected = -cfg.reward_weights.penalty_narrow_stance * normalized_deficit * env.dt

    assert contribution == pytest.approx(expected, abs=1e-6)
