...
def test_smoke13_action_scale_spec(smoke13_cfg) -> None:
    """TB-scalar 0.25 baseline (mjx_config.py:103) with per-joint
    widening only on the leg-pitch chain.  Pinning the exact map so a
    future config rewrite can't drift to either pure TB scalar (too
    tight for WR's longer legs) or smoke12b's wider {knee 0.667, hip
    0.353/0.387} cap."""
    env = smoke13_cfg.env
    ...
    assert got == pytest.approx(expected), (
        f"smoke13 per-joint scale must be {expected}; got {got}"
    )
