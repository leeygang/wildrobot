...
- **Run**: `training/wandb/offline-run-20260606_091345-j6kaw1ij`
- **Checkpoints**: `training/checkpoints/ppo_walking_v0210_smoke9_exact_signed_vy_v00210_20260606_091415-j6kaw1ij`
- **Config**: `ppo_walking_v0210_smoke9_exact_signed_vy.yaml`
- **Authoritative result**: `selected_checkpoint_path = null`

Smoke9 did **not** produce a promotable checkpoint. The deterministic
post-training gate blocked every top candidate. The common failure is still
`lateral_probe_tracking`: the policy moves in positive heading-local `vy` under
both positive and negative lateral commands.

Top candidates:

| checkpoint | straight eval `[0.13,0,0]` | `+vy` probe `[0.13,+0.065,0]` | `-vy` probe `[0.13,-0.065,0]` | verdict |
|---|---|---|---|---|
| `checkpoint_2010_41164800.pkl` | `forward_velocity 0.197`, `cmd_err 0.0713`, `ratio 1.52` **fail**, `step_length 0.170`, `lateral_velocity_abs 0.175` **fail**, `world_y_drift_abs_m 0.176` | achieved `+0.0414`, signed ratio `+0.64` **PASS** | achieved `+0.0456`, signed ratio `-0.70` **FAIL** | best candidate, still one-sided +`vy` |
| `checkpoint_2060_42188800.pkl` | `forward_velocity 0.201`, `cmd_err 0.0830` **fail**, `ratio 1.55` **fail**, `step_length 0.152`, `lateral_velocity_abs 0.154` **fail**, `world_y_drift_abs_m 0.662` **fail** | achieved `+0.0455`, signed ratio `+0.70` **PASS** | achieved `+0.0412`, signed ratio `-0.63` **FAIL** | fast gait, wrong sign |
| `checkpoint_1960_40140800.pkl` | `forward_velocity 0.182`, `cmd_err 0.0578`, `ratio 1.40`, `step_length 0.153`, `lateral_velocity_abs 0.166` **fail**, `world_y_drift_abs_m 0.810` **fail** | achieved `+0.0320`, signed ratio `+0.49` **FAIL** | achieved `+0.0290`, signed ratio `-0.45` **FAIL** | lower `vy`, but still positive under both signs |

### What smoke9 validated
- **The exact signed-`vy` sampler is active.** The train-side mean
  `tracking/velocity_cmd_vy_abs` moved from smoke8's `~0.026` to smoke9's
  `~0.041`, matching a balanced `{ -0.065, 0, +0.065 }` mixture
  (`(2/3) * 0.065 = 0.043` before zero/branch effects).
- **The instrumentation fix worked.** `reward/cmd_lateral_velocity_track`
  appears in the smoke9 metrics and exposes the lateral factor directly.
- **The failure is behavioral, not a probe bug.** Positive-`vy` probes pass or
  nearly pass; negative-`vy` probes still achieve positive `vy`.

### Why smoke9 failed (code + data)
1. **The sampler hypothesis was necessary but not sufficient.**
   Smoke8 mostly trained below the probe magnitude because TB's ellipse
   projected `vy` by `cos(theta)`. Smoke9 fixed that by sampling exact signed
   bins. The policy still kept the inherited positive-lateral attractor, so
   insufficient command exposure is no longer the only root cause.

2. **The lateral factor is logged, but not independently rewarded.**
   `training/envs/wildrobot_env.py::_cmd_forward_velocity_track_reward` still
   returns one combined 2D exponential:

   `exp(-(alpha_x * vx_err^2 + alpha_y * vy_err^2))`

   `reward/cmd_lateral_velocity_track` is currently a diagnostic factor, not a
   separate weighted term in `_aggregate_reward`. When the combined velocity
   reward is already small, the optimizer can get more return by preserving the
   forward gait and other imitation terms than by flipping lateral sign.

3. **Training longer on smoke9 is low-yield.**
   The deterministic top candidates are early (`1960-2060`). Late train metrics
   reduce stride instead of discovering signed lateral control:
   - smoke9 first-10 train rows: `forward_velocity ~0.082`,
     `step_length ~0.060`, `cmd_vy_err ~0.109`
   - smoke9 last-10 train rows: `forward_velocity ~0.060`,
     `step_length ~0.039`, `cmd_vy_err ~0.111`

### ToddlerBot and public-reference comparison
- ToddlerBot samples walk commands with the same ellipse projection
  (`~/projects/toddlerbot/toddlerbot/locomotion/walk_env.py`) and uses a tight
  2D velocity reward (`walk.gin`: `RewardScales.lin_vel_xy = 2.0`,
  `RewardsConfig.lin_vel_tracking_sigma = 1000.0`;
  `mjx_env.py::_reward_lin_vel_xy`).
- WR's exact signed-`vy` sampler is an intentional curriculum deviation from TB:
  TB's broad ellipse is appropriate once the policy owns signed 2D velocity,
  while WR is still stuck in a one-sided lateral local minimum.
- The next step still follows the command-conditioned locomotion pattern used
  by PPO locomotion systems (Schulman et al., 2017; Rudin et al., 2021), and
  keeps the reference-state warm start / imitation structure consistent with
  DeepMimic-style RSI (Peng et al., 2018). The WR-specific change is to split
  `vx` and `vy` rewards so the lateral sign gradient cannot be hidden inside a
  near-zero product term.

### Smoke10 plan - axis-split command velocity reward
- **Resume from** the smoke6 promoted checkpoint again, not smoke8/smoke9:
...
- **Keep**:
  ...
  - exact signed-`vy` bins `{ -0.065, 0, +0.065 }`
  - `min_velocity_y = -0.065`, `max_velocity_y = +0.065`
  - eval command `[0.13, 0.0, 0.0]`
  - probes `[0.13, +0.065, 0.0]` and `[0.13, -0.065, 0.0]`
  - yaw off (`max_yaw_rate = 0`, `cmd_yaw_rate_track = 0`)
  - strict lateral drift + signed-probe gates
- **Change the reward structure:**
  - set `cmd_velocity_track_dim: 1` so `cmd_forward_velocity_track` is a pure
    forward `vx` reward again
  - add a new separately weighted lateral command reward, e.g.
    `reward_weights.cmd_lateral_velocity_track: 1.0`
  - compute that term from the existing lateral factor:

    `exp(-alpha_y * (vy_actual - cmd_vy)^2)`

  - keep `alpha_x = 562.5` and `alpha_y = 120.0`
  - preserve a raw lateral-factor diagnostic so smoke10 can still distinguish
    "lateral factor is high" from "weighted contribution is high"

### Smoke10 implementation notes
- Add `cmd_lateral_velocity_track: float = 0.0` to
  `training/configs/training_runtime_config.py::RewardWeightsConfig`.
- Parse it in `training/configs/training_config.py::_parse_reward_weights_config`
  so strict YAML loading accepts the new key.
- In `training/envs/wildrobot_env.py`, compute the lateral factor whenever
  `cmd_velocity_track_dim == 2` **or** `cmd_lateral_velocity_track > 0`.
  Existing configs with `cmd_velocity_track_dim == 1` and lateral weight `0`
  should keep the neutral factor behavior.
- Add the weighted lateral term to `_aggregate_reward`.
- Add/update metrics carefully:
  - keep the existing `reward/cmd_lateral_velocity_track` raw factor diagnostic,
    or explicitly rename/document if it becomes the weighted contribution
  - if needed, add a separate weighted metric such as
    `reward/cmd_lateral_velocity_track_contrib`
- Add a smoke10 config copied from smoke9 with only the axis-split reward
  changes above.
- Add tests that pin:
  - smoke9 config remains unchanged
  - smoke10 config loads and has exact signed-`vy`, `cmd_velocity_track_dim=1`,
    and nonzero `cmd_lateral_velocity_track`
  - the lateral reward is non-neutral under `cmd_velocity_track_dim=1` when the
    new weight is nonzero
  - reward/logging allow-lists cover any new metric key

### Recommendation after smoke9
- **Do not deploy smoke9.**
- **Do not resume smoke9.** It reinforced the same positive-lateral mode.
- **Do not only increase `alpha_y` or train longer.** Smoke9 already put the
  exact signed commands into training and still failed the negative probe.
- **Run smoke10 as an axis-split reward experiment from smoke6.** This is the
  smallest next change that attacks the observed failure: the lateral sign
  objective needs its own live reward channel while the forward gait remains
  protected by the validated smoke6 forward reward.

## [v0.21.0-smoke8-result + smoke9-plan] - 2026-06-06: stronger `vy` range still learned one-sided lateral mode; smoke9 = exact signed-`vy` curriculum + instrumentation fix

### Smoke8 result — no deployable checkpoint
- **Run**: `training/wandb/offline-run-20260602_221706-gbis9wc4`
- **Checkpoints**: `training/checkpoints/ppo_walking_v0210_smoke8_2d_signed_vy_v00210_20260602_221820-gbis9wc4`
- **Config**: `ppo_walking_v0210_smoke8_2d_signed_vy.yaml`
- **Authoritative result**: `selected_checkpoint_path = null`

Smoke8 did **not** produce a promotable checkpoint. The deterministic
post-training gate blocked every top candidate on `lateral_probe_tracking`, and
also on some combination of `cmd_vs_achieved_forward`,
`forward_velocity_cmd_ratio`, `lateral_velocity_abs`, and
`world_y_drift_abs_m`.
...
| checkpoint | straight eval `[0.13,0,0]` | `+vy` probe `[0.13,+0.065,0]` | `-vy` probe `[0.13,-0.065,0]` | verdict |
|---|---|---|---|---|
| `checkpoint_2060_42188800.pkl` | `forward_velocity 0.187`, `cmd_err 0.0826` **fail**, `step_length 0.119`, `lateral_velocity_abs 0.193` **fail**, `world_y_drift_abs_m 0.646` **fail** | achieved `+0.0754`, signed ratio `+1.16` **PASS** | achieved `+0.0741`, signed ratio `-1.14` **FAIL** | one-sided +`vy`, forward overshoot/drift |
| `checkpoint_2560_52428800.pkl` | `forward_velocity 0.163`, `cmd_err 0.0526`, `step_length 0.130`, `lateral_velocity_abs 0.177` **fail**, `world_y_drift_abs_m 0.143` | achieved `+0.0443`, signed ratio `+0.68` **PASS** | achieved `+0.0396`, signed ratio `-0.61` **FAIL** | best path drift, still wrong sign |
| `checkpoint_2860_58572800.pkl` | `forward_velocity 0.217`, `cmd_err 0.0914` **fail**, `ratio 1.67` **fail**, `step_length 0.177`, `lateral_velocity_abs 0.159` **fail**, `world_y_drift_abs_m 0.635` **fail** | achieved `+0.0450`, signed ratio `+0.69` **PASS** | achieved `+0.0460`, signed ratio `-0.71` **FAIL** | fast gait, still wrong sign |

**Conclusion:** smoke8 preserved forward walking but still failed signed 2D
command ownership. The policy again selected a positive-lateral attractor under
both positive and negative lateral commands. This is a real behavioral result,
not a broken eval probe: the configured `±0.065` probes ran and blocked
promotion as intended.

### What smoke8 validated in code
- **The smoke8 config changes were live:**
  - `min_velocity_y = -0.065`
  - `max_velocity_y = +0.065`
  - `cmd_velocity_track_dim = 2`
  - `cmd_forward_velocity_alpha_y = 120.0`
  - probes `[0.13, ±0.065, 0.0]`
- **The anisotropic 2D reward compares actual `(vx, vy)` to commanded
  `(cmd_vx, cmd_vy)`** in
  `training/envs/wildrobot_env.py::_cmd_forward_velocity_track_reward`, with:
  - `alpha_x = 562.5`
  - `alpha_y = 120.0`
- **The lateral probe gate is functioning.** It rejects wrong-sign behavior even
  when the absolute lateral speed is near the commanded magnitude.

### Why smoke8 failed (code + data)
1. **The train-side lateral command was still much smaller than the eval probe.**
   Smoke8 widened the nominal lateral range to `±0.065`, but kept the
   TB-style walk ellipse:

   `vy = uniform(deadzone_y, |y_bound|) * cos(theta)`

   With `deadzone_y = 0.02` and `|y_bound| = 0.065`, the expected mean
   `|vy_cmd|` is only about `0.027`. The run confirms this:
   - smoke7 late `tracking/velocity_cmd_vy_abs ≈ 0.015`
   - smoke8 late `tracking/velocity_cmd_vy_abs ≈ 0.025`

   So smoke8 evaluated at `|vy| = 0.065`, but most training updates saw a much
   weaker signed lateral command. The existing positive-`vy` attractor survived.

2. **Increasing `alpha_y` alone created reward separation, but not enough
   signed examples.**
   At `alpha_y = 120`, wrong-sign behavior under the `-0.065` probe is now
   expensive:
   - candidate `2560`: `cmd_vy=-0.065`, achieved `+0.0396` ->
     `vy_err≈0.105`, lateral factor `exp(-120*err^2)≈0.27`
   - candidate `2060`: `cmd_vy=-0.065`, achieved `+0.0741` ->
     `vy_err≈0.139`, lateral factor `≈0.10`

   The reward is no longer too soft. The issue is exposure: the training
   sampler does not put enough probability mass on the exact signed cases that
   the promotion probes require.

3. **Training longer on smoke8 is low-yield.**
   Later train-side metrics reduce lateral speed but also degrade gait quality:
   by iter `3400`, `forward_velocity≈0.041`, `step_length≈0.0197`, while
   action saturation rises to `≈0.034`. That is not a clean signed-2D gait; it
   is the policy giving up stride while trying to reduce lateral error.

4. **Instrumentation bug: lateral factor is produced by env code but dropped by
   W&B tracking.**
   `training/envs/wildrobot_env.py` emits
   `reward/cmd_lateral_velocity_track`, but
   `training/core/experiment_tracking.py::REWARD_TERM_KEYS` does not include it,
   so `metrics.jsonl` lacks the lateral-factor diagnostic. `tracking/cmd_vy_err`
   is present, but the next 2D run should log the reward factor explicitly.

### ToddlerBot comparison
- TB's action path builds gait from a static default/home target:
  `motor_target = default_action + 0.25 * action`, with final clipping only at
  motor limits (`~/projects/toddlerbot/toddlerbot/locomotion/mjx_env.py`).
  Smoke8 correctly keeps WR on the validated smoke6 `home + RSI +
  coverage-1.1` contract.
- TB's walk sampler also uses an ellipse-projected `vy`
  (`~/projects/toddlerbot/toddlerbot/locomotion/walk_env.py`), and TB's
  velocity reward is 2D command tracking (`walk.gin`: `lin_vel_xy = 2.0`,
  `lin_vel_tracking_sigma = 1000`).
- **WR-specific rationale for diverging in smoke9:** TB's broad ellipse is a
  good coverage sampler once 2D command ownership exists. WR currently has a
  confirmed one-sided lateral local minimum, so the next stage should
  over-sample the exact signed lateral cases before returning to the broader TB
  sampler.

### Smoke9 plan — exact signed-`vy` curriculum from smoke6
- **Resume from** the smoke6 promoted checkpoint again, not smoke8:
  - `training/checkpoints/ppo_walking_v0210_smoke6_home_rsi_v00210_20260601_074049-fayqouf3/eval_promoted/checkpoint_1900_38912000.pkl`
- **Keep**:
  - `loc_ref_residual_base: home`
  - `loc_ref_rsi_enabled: true`
  ...
  - `alpha_y = 120.0`
  - yaw off (`max_yaw_rate = 0`, `cmd_yaw_rate_track = 0`)
  - strict lateral drift + signed-probe promotion gates
- **Change the command curriculum only:**
  - replace the ellipse-projected lateral walk sampler for this stage with
    balanced exact signed bins, e.g.
    - `[0.13, -0.065, 0.0]`
    - `[0.13,  0.000, 0.0]`
    - `[0.13, +0.065, 0.0]`
  - a narrow forward band around `vx≈0.13` is acceptable, but do not let the
    lateral command magnitude collapse through `cos(theta)` during this stage
  - keep the same eval probes `[0.13, ±0.065, 0.0]`
- **Fix instrumentation before launching smoke9:**
  - add `reward/cmd_lateral_velocity_track` to
    `training/core/experiment_tracking.py::REWARD_TERM_KEYS`
  - verify the key appears in the smoke9 offline `metrics.jsonl`

### Recommendation after smoke8
- **Do not deploy smoke8.**
- **Do not resume smoke8.** It learned the same one-sided lateral mode as
  smoke7.
- **Run smoke9 from smoke6's promoted checkpoint with exact signed-`vy`
  command bins.** This is the smallest change that attacks the observed failure:
  insufficient train-side exposure to the same signed lateral commands used by
  the authoritative probes.
...
- **Residual scales**: **symmetric per L/R pair**, sized to cover the prior's max swing-from-home over the **full command range / all library bins** (NOT just the single-vx 0.1333 `derive_residual_scales.py` point, which undercovers high-vx bins): **hip_pitch 0.64** (all-bins max 0.601), **knee 0.978** (0.889), **ankle_pitch 0.33** (0.308), hip_roll/ankle_roll 0.375. The old 0.3534/0.3867 L/R split was a spurious derivation artifact. Keep the ±1 clip (wide, not fully unbounded).
- **Reachability hard-guard** (`wildrobot_env.py::_assert_rsi_reachable_from_home`): env init now FAILS if `home`+RSI is paired with scales that don't cover the prior's per-joint swing on every bin — closes the off-manifold hole (a static base + undersized scales would snap the RSI pose at step 0). A general all-bins test + a negative test pin it.
...
- **Prerequisites RESOLVED this session**: (a) `feet_phase` ceiling is adequate — perfect-tracking 0.774 mean → 0.116/step weighted (largest term); the `max(0,raw−flat)` subtraction zeroes the flat-foot exploit, full reward is clean step gradient (NOT a blocker). (b) stride reachable from home (FK 0.27 m, 9× the gate). (c) smoke5 deterministic-eval confirms the q_ref variant already walks — so the home variant has a working fallback.
...
- **Levers vs smoke3**: `cmd_sampler_3d_branched: false` (legacy scalar `(vx,0,0)`); `cmd_velocity_track_dim: 1` (forward-only reward — the main fix); `cmd_yaw_rate_track: 0`; `min/max_velocity_y: 0`; `max_yaw_rate: 0`; `cmd_turn_chance: 0`; `eval_velocity_cmd: [0.13, 0, 0]` (reachable forward, exact bin, G4 floor 0.065); `eval_velocity_cmd_probes: []` (the probe evaluator is lateral/yaw-only — a forward probe would be mis-typed; forward generalization read from training metrics). **Kept**: floor 0.065 / max 0.26 / interval 0.065 (0.26 bin), α 562.5, action_rate −1.0, feet_phase, home anchor, obs_v8, 3D library, critic actor+privileged.
...
### Why (root cause of the smoke2 miss, code + trajectory confirmed)
...
## [v0.21.0-smoke2-`5v4j5j7w`-result] - 2026-05-29: sampler-fixed re-run → dead-gradient standing/tap minimum at the fast band

- **Run**: `training/wandb/offline-run-20260529_115206-5v4j5j7w` (config `ppo_walking_v0210_smoke2.yaml` — the **re-run of the smoke2 config after the `ee399ee` sampler fix**; the first smoke2 run `yxfkg6wh` is the entry below). This run executed the `yxfkg6wh` entry's smoke3 plan (sampler fixed, band kept at 0.18/0.26) and is the empirical falsification of it.
- **Checkpoint dir**: `training/checkpoints/ppo_walking_v0210_smoke2_v00210_20260529_115226-5v4j5j7w`.
- **Stopped at iter 660 / 13.5M env-steps** (44% of the 1500-iter / ~30.7M target; no crash/NaN in logs — manual stop or host session end). No `Evaluate/*` namespace logged → checkpoint selection is **non-authoritative**; do not deploy.
- **Sampler sanity (skill pre-flight #2): `env/velocity_cmd = 0.197` vs YAML midpoint 0.22 (Δ ≈ 10%, in tolerance).** The `ee399ee` vx-clamp / positive-only fix **worked** — this is NOT a sampler-projection regression (unlike the prior `yxfkg6wh` run, whose cmd had collapsed to 0.081).

### Verdict: dead-gradient standing / tap-in-place minimum (slight backward drift)

| Gate | Metric | Value | Result |
|---|---|---:|---|
| G4 | `env/forward_velocity` | −0.072 | **FAIL** (negative / backward) |
| G4 | `env/episode_length` | 1000 | pass-but-meaningless (full survival by not walking) |
| G4 | `tracking/cmd_vs_achieved_forward` | 0.29 | **FAIL** (≫ 0.075) |
| G4 | `tracking/step_length_*_event_m` | −0.0014 | **FAIL** (≈ 0, slightly backward) |
| G5 | `residual_{hip,knee}_{l,r}_abs` | all ≤ 0.23 | pass — but vacuous (policy barely moves) |
| G5 | `forward_velocity_cmd_ratio` | −6.1 | n/a (nonsense at fv < 0) |
| G7 | beat bare-q_ref baseline (fv) | −0.072 vs −0.09 | **FAIL** (same backward sign) |
...
- `ref/feet_pos_err_l2 = 3.4 m`, `ref/torso_pos_xy_err = 3.6 m` — **expected/harmless**: those terms have weight 0 (prior-free recipe); they are unweighted diagnostics of divergence from a prior the reward ignores. (See `walking_training.md` Reward Design active-config drift callout, added in `c56e8e6`.)
- `ref/contact_phase_match = 0.29` (below 0.80) — `ref_contact_match` disabled, so not gating; report-only.
- PPO `clip_fraction = 0.50`, `approx_kl = 0.053` — elevated (gradient dominated by feet_phase + penalties with velocity dead); symptom, not cause.

### ToddlerBot alignment
...
## [v0.20.1-smoke14-13zk1p2v-diagnosis] - 2026-05-24: survives with phase activity, but forward walking collapses

### Run

- W&B offline run:
  `training/wandb/offline-run-20260523_175517-13zk1p2v`
- Checkpoints:
  `training/checkpoints/training_config_v00201_20260523_175523-13zk1p2v`
- Post-training deterministic eval:
  `training/checkpoints/training_config_v00201_20260523_175523-13zk1p2v/post_training_eval_summary.json`
- Config verified from the checkpoint:
  `min_velocity=0.18`, `max_velocity=0.26`,
  `eval_velocity_cmd=0.26`, `loc_ref_offline_command_vx=0.26`,
  `cmd_velocity_track_dim=1`, `critic_obs_history_frames=15`,
  `loc_ref_reset_base=home`, `loc_ref_residual_base=home`.

### Verdict

**Do not deploy this run.**  Smoke14 does not extend the smoke12b
`ot8zazo1` walking gait.  It survives the 500-step horizon at late
checkpoints, but the useful forward gait collapses into low-progress
foot-phase activity with large lateral motion.

The cleanest read is:

- smoke14 asks for about `0.22 m/s` during train and `0.26 m/s` in
  post-training eval, roughly twice the smoke12b `ot8zazo1` command,
- the policy learns to collect more `feet_phase` reward by moving the feet,
  but does not translate that into healthy forward stride,
- and deterministic post-training eval rejects all top candidates.

### Source-of-truth check

Code and data checked before writing this diagnosis:

- WR command velocity reward semantics:
  `training/envs/wildrobot_env.py:_cmd_forward_velocity_track_reward`
- WR carry-proxy and per-foot step metrics:
  `training/envs/wildrobot_env.py` around
  `tracking/step_length_touchdown_event_m` and
  `tracking/step_length_{left,right}_event_m`
- WR post-training eval gates:
  `training/core/post_training_eval.py`
- Smoke14 config:
...
| Iter | Episode length | Forward velocity | Command | Cmd error | Cmd ratio | Lateral velocity | Carry step proxy | Read |
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| `760` | `193` | `0.123` | `0.219` | `0.105` | `0.57` | `0.210` | `-2.8 mm` | unstable, moving but not stepping forward |
| `860` | `500` | `0.136` | `0.219` | `0.110` | `0.63` | `0.210` | `-3.7 mm` | best train `vx`, but step length is backward / near zero |
| `1380` | `483` | `0.133` | `0.220` | `0.115` | `0.61` | `0.234` | `10.3 mm` | best post-eval candidate by train score, still short-step |
| `1460` | `500` | `0.079` | `0.220` | `0.155` | `0.36` | `0.209` | `12.1 mm` | analyzer fallback best, but tracking fails |
| `1500` | `500` | `~0.000` | `0.220` | `0.227` | `~0.00` | `0.204` | `14.8 mm` | survives while forward walking collapses |

### Deterministic post-training eval

Post-training eval tested the top 3 train-side candidates and found no
promotion candidate.

| Candidate | Eval forward velocity | Eval cmd error | Eval ratio | Eval episode length | Eval carry step proxy | Failed gates |
|---|---:|---:|---:|---:|---:|---|
| `checkpoint_1380_28262400.pkl` | `0.0295` | `0.2312` | `0.114` | `500` | `15.2 mm` | `vx`, cmd error, step, ratio |
| `checkpoint_1410_28876800.pkl` | `0.0292` | `0.2442` | `0.112` | `481` | `17.6 mm` | `vx`, cmd error, step, ratio |
| `checkpoint_1360_27852800.pkl` | `0.0303` | `0.2319` | `0.117` | `500` | `20.6 mm` | `vx`, cmd error, step, ratio |

So the train-side `0.13 m/s` moments do not survive deterministic pinned
evaluation.  The eval result is a low-forward-progress behavior, not a
walking candidate.

### Gait diagnosis vs smoke12b `ot8zazo1`

| Metric | smoke12b `ot8zazo1` @ iter 2450 | smoke14 @ iter 1500 | Read |
|---|---:|---:|---|
| Command | `0.106 m/s` | `0.220 m/s` | smoke14 asks about 2.1x harder |
| Forward velocity | `0.0968 m/s` | `~0.000 m/s` | regressed |
| Cmd error | `0.0306 m/s` | `0.227 m/s` | regressed badly |
| Cmd ratio | `0.934` | `~0.00` | G5 ratio fail |
| Carry step proxy | `49.5 mm` | `14.8 mm` | regressed |
| Per-event step L / R | `31 / 70 mm` | `13 / 10 mm` | smoke14 is tiny symmetric shuffling |
| Swing time L / R | `158 / 395 ms` | `165 / 152 ms` | more symmetric, but too short |
| `ref/contact_phase_match` | `0.306` | `0.336` | still poor |
| `reward/feet_phase` | `0.0868` | `0.1028` | more foot-phase reward without useful stride |
| `tracking/residual_knee_left_abs` | `0.214` | `0.321` | G5 residual fail worsened |

The important distinction is that smoke12b `ot8zazo1` had an ugly,
asymmetric gait that still produced real forward stepping.  Smoke14 is
more symmetric late in training, but the symmetry is not healthy gait
symmetry; it is mostly both feet taking very small steps while the body
does not make forward progress.

### Carry-proxy metric caveat

`tracking/step_length_touchdown_event_m` is useful as a dashboard signal,
but it is not the exact mean step length.  The env code labels it as a
**carry proxy**: on a touchdown, the metric is updated to that touchdown's
step length, and then the same value is carried forward on non-touchdown
steps until the next touchdown.

That means its rollout mean is biased by how long each last touchdown value
is carried.  It can answer "what is the recent carried touchdown step
length?", but it cannot answer "what was the average step length over actual
touchdown events?"

For event-accurate diagnosis, use:

```text
per-event step length = tracking/step_length_{left,right}_event_m
                      / tracking/touchdown_rate_{left,right}_count
```

At smoke14 iter `1500`, the logged values are:

| Foot | Logged event sum mean | Touchdown rate | Per-touchdown step |
|---|---:|---:|---:|
| Left | `0.000641 m` | `0.04795` | `13.4 mm` |
| Right | `0.000343 m` | `0.03335` | `10.3 mm` |

So the carry proxy says `14.8 mm`, while the event-accurate per-foot read
says the actual touchdown steps are about `10-13 mm`.  Both are below the
G4 `30 mm` minimum and far below the smoke14 command-conditioned target.
For reference, the changelog's WR/TB kinematic ledger uses
`step length = vx * cycle_time / 2`; at smoke14's `0.22 m/s` train command
and the WR `~0.96 s` cycle assumption used in the ledger, healthy
command-matched stepping would be about `106 mm` per touchdown.  Smoke14 is
therefore not merely "a little short"; it is around one-tenth of the
command-scaled stride.

### System / training-flow gaps exposed

1. `ppo.eval.enabled=false` leaves the run without an in-training
   `Evaluate/*` series even though the training loop has the correct
   ToddlerBot-style eval namespace available.
2. The offline analyzer falls back to `env/episode_length` when no
   `Evaluate/*` exists, which selected `checkpoint_1460_29900800.pkl`
   even though post-training deterministic eval rejected every top
   candidate.
3. Smoke14's `cmd_velocity_track_dim=1` ignores lateral velocity in the
   reward.  The run then carries `tracking/lateral_velocity_abs ≈ 0.20 m/s`,
   comparable to the forward command itself.
4. The run metadata is easy to misread: the source smoke14 YAML describes
   `2048` envs for `750` iterations, but the checkpoint config and W&B
   config show the active run used `1024` envs for an effective `1500`
   logged iterations to reach the same `30.72M` env steps.

### ToddlerBot comparison

ToddlerBot's active recipe uses dense `feet_phase`, `penalty_pose`,
`penalty_feet_ori`, `lin_vel_xy`, and `penalty_action_rate`
(`walk.gin:111-131`), with foot-height phase tracking implemented in
`walk_env.py:631-695`.  Smoke14 has the matching reward family present,
but the behavior is not ToddlerBot-like: the policy collects more
`feet_phase` than smoke12b while losing forward progression.  In other
words, the active reward family is being used to move the feet, not to
produce a command-tracking alternating walk.
...
- forward velocity,
- `touchdown_rate_left/right_count`,
...
- **Stride structure:** both feet are short, not one-sided.  The per-foot
  ...
  `step_length_left_event_m / touchdown_rate_left_count = 0.0207 m` and
  `step_length_right_event_m / touchdown_rate_right_count = 0.0264 m`; both remain
...
5. **Per-foot stride / swing-time diagnostics**
   ...
   per-event mean = ``<value>_event / touchdown_rate_<side>_count``
   post-aggregation.
