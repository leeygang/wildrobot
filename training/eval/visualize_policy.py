...
Usage:
    ...
    uv run python training/eval/visualize_policy.py --headless --record output.mp4
...
def resolve_checkpoint_path(checkpoint_arg: str | None) -> Path | None:
    ...
    # Try relative to project root with training prefix stripped/added
    if str(path).startswith("training/"):
        without_prefix = project_root / Path(
            str(path).replace("training/", "", 1)
        )
        if without_prefix.exists():
            return without_prefix.resolve()
    else:
        with_prefix = project_root / "training" / path
    ...
    if headless:
        ...
        try:
            ...
            if "mjpython" in str(e) and is_macos:
                ...
                print(
                    "   Run with: uv run mjpython training/eval/visualize_policy.py ..."
                )
                print(
