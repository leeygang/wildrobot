    ...
    *,
    ...
    return {
        ...
        "torque_saturation_frac": (
            float(np.mean(ratio > 0.95)) if ratio.size else 0.0
        ),
        "policy_action_signed_mean": (
            float(np.mean(policy)) if policy.size else 0.0
        ),
        "policy_action_abs_mean": (
            float(np.mean(abs_policy)) if policy.size else 0.0
        ),
...
def summarize_roll_load_sharing(
    *,
    ...
    step_arrays = (left_loaded, right_loaded, dones, truncations)
    if any(array.shape != step_shape for array in step_arrays):
        raise ValueError("support/done arrays must have shape (T, N)")
    if ctrl_dt <= 0.0:
    ...
    for window_name, window_mask in windows.items():
        ...
        for phase_name, support_mask in support_masks.items():
            ...
            window_output["support_phases"][phase_name] = {
...
def main() -> int:
    ...
    env_state = jax.vmap(env.reset_for_eval)(
        jax.random.split(rng, args.num_envs)
    )
