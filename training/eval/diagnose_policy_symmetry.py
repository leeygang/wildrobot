...
# Sign is expressed in joint coordinates after swapping left/right.  The values
# follow from the actuator axes in assets/v2/mujoco_robot_config.json under the
# axial-vector reflection rule a' = -diag(1, -1, 1) a.
_PAIRED_JOINT_SIGNS = {
    "shoulder_pitch": -1.0,
    "shoulder_roll": -1.0,
    "elbow_pitch": 1.0,
...
def joint_mirror_transform(
    ...
    """Return ``(source_index, sign)`` for a sagittal joint reflection.

    The returned arrays satisfy ``mirrored[..., i] = sign[i] * x[..., source[i]]``.
    The active WR 17-DoF contract has one self-mirrored waist-yaw joint and
    eight left/right pairs.
    """
...
def mirror_observations(obs: jax.Array, spec: PolicySpec) -> jax.Array:
    ...
    if spec.observation.layout_id != "wr_obs_v8_cmd3d":
        raise ValueError(
            "Policy symmetry diagnostic supports wr_obs_v8_cmd3d only; got "
    ...
    # Polar vector under y reflection.
    gravity = values[..., fields["gravity_local"]]
    mirrored = mirrored.at[..., fields["gravity_local"]].set(
        gravity * jnp.asarray([1.0, -1.0, 1.0], dtype=jnp.float32)
    )
    # Angular velocity is an axial vector: det(S) * S * omega.
    angvel = values[..., fields["angvel_heading_local"]]
    mirrored = mirrored.at[..., fields["angvel_heading_local"]].set(
        angvel * jnp.asarray([-1.0, 1.0, -1.0], dtype=jnp.float32)
    )
    ...
    offset = 7
    for _ in range(3):
        block = history[..., offset : offset + action_dim]
        mirrored_history = mirrored_history.at[
            ..., offset : offset + action_dim
        ].set(mirror_actions(block, spec))
        offset += action_dim
