...
# This is an interactive diagnostic script, not a pytest test module.
# Keep it import-safe so `pytest` collection doesn't try to execute it.
__test__ = False
...
def main() -> None:
    ...
    for i in range(model.nu):
        ...
        print(
            f"{i}: {name:20s} ctrl=[{ctrl_range[0]:6.2f}, {ctrl_range[1]:6.2f}] "
    ...
    for jname in joint_names:
        ...
        if jid >= 0:
            ...
            print(
                f"{jname:20s}: [{np.degrees(jnt_range[0]):6.1f}°, {np.degrees(jnt_range[1]):6.1f}°]"
            )
    ...
    print(
        f"Left knee actual: {left_knee_pos:.3f} rad ({np.degrees(left_knee_pos):.1f}°)"
    )
    print(
        f"Right knee actual: {right_knee_pos:.3f} rad ({np.degrees(right_knee_pos):.1f}°)"
    )

    # Check torques used
    print(
        "\nActuator forces (left_knee, right_knee): "
        f"{data.actuator_force[2]:.2f}, {data.actuator_force[6]:.2f}"
    )
    print("Force limit: ±4.0 Nm")
    ...
    print(
        f"Right hip pitch: {right_hip_pos:.3f} rad ({np.degrees(right_hip_pos):.1f}°)"
    )
    ...
    print(
        f"Left knee: {data.qpos[7+2]:.3f} rad ({np.degrees(data.qpos[7+2]):.1f}°)"
    )
    print(
        f"Right knee: {data.qpos[7+6]:.3f} rad ({np.degrees(data.qpos[7+6]):.1f}°)"
    )
