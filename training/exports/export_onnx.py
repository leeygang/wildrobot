#!/usr/bin/env python3
"""Export a `training` PPO checkpoint (.pkl) to a deterministic ONNX policy.

This exporter targets the PPO policy produced by `training/train.py`.
...
Usage:
  uv run python training/exports/export_onnx.py \
    --checkpoint training/checkpoints/ppo_standing_v00113_final.pkl \
    --output training/checkpoints/ppo_standing_v00113_final.onnx
...
def export_checkpoint_to_onnx(checkpoint_path: Path, output_path: Path) -> None:
    ...
    model = helper.make_model(
        graph,
        producer_name="training.export_onnx",
