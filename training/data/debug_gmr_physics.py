...
Usage:
    ...
    uv run python scripts/verify_gmr_physics.py \
        --motion training/data/gmr/walking_medium01.pkl \
        --render
"""
...
def run_physics_test(
    ...
    Args:
        ...
        z_offset: Z offset to apply to root position (use negative value to lower robot)
    """
    # Load motion
    ...
    # Compute num_frames if not present
    if "num_frames" not in motion:
        motion["num_frames"] = len(motion["dof_pos"])
    if "duration_sec" not in motion:
        motion["duration_sec"] = motion["num_frames"] / motion["fps"]
    ...
    for j in range(n_actuators):
        ...
        below = dof_pos[:, j] < lo
        above = dof_pos[:, j] > hi
        violations += np.sum(below) + np.sum(above)
        dof_pos[:, j] = np.clip(dof_pos[:, j], lo, hi)
    if violations > 0:
        print(f"  Warning: Clamped {violations} joint limit violations in GMR motion")
    ...
    # GMR stores quaternion as (x, y, z, w), MuJoCo expects (w, x, y, z)
    ref_root_rot_xyzw = motion["root_rot"]
    ref_root_rot = np.zeros_like(ref_root_rot_xyzw)
    ref_root_rot[:, 0] = ref_root_rot_xyzw[:, 3]  # w
    ref_root_rot[:, 1] = ref_root_rot_xyzw[:, 0]  # x
    ref_root_rot[:, 2] = ref_root_rot_xyzw[:, 1]  # y
    ref_root_rot[:, 3] = ref_root_rot_xyzw[:, 2]  # z
    ...
    # Set initial pose and find minimum foot height
    if harness_mode == "grounded" and z_offset == 0.0:
        ...
        # Find minimum foot Z
        foot_geom_names = ['left_toe', 'right_toe', 'left_heel', 'right_heel']
        min_foot_z = float('inf')
        for name in foot_geom_names:
            try:
                gid = mujoco.mj_name2id(mj_model, mujoco.mjtObj.mjOBJ_GEOM, name)
                min_foot_z = min(min_foot_z, mj_data.geom_xpos[gid][2])
            except:
                pass

        if min_foot_z > 0.001:  # Feet are above ground
            z_offset = -min_foot_z + 0.001  # Lower to just above ground
            print(f"  Auto Z-offset: {z_offset:.4f}m (feet were {min_foot_z:.4f}m above ground)")
    ...
    print(f"\nRunning physics simulation ({num_frames} frames){'  [LOOP MODE]' if loop else ''}...")
    ...
    try:
        ...
        while True:
            ...
            if loop and loop_count > 1:
                ...
                mujoco.mj_forward(mj_model, mj_data)
                sim_xy_start = mj_data.qpos[0:2].copy()
                tracking_errors.clear()
                harness_forces.clear()
                contact_forces.clear()
                heights.clear()
                pitches.clear()
                rolls.clear()
                print(f"\n  [Loop {loop_count}] Restarting simulation...")

            # For grounded mode: initialize robot at GMR starting pose with z_offset
            if harness_mode == "grounded":
            ...
            for i in range(num_frames):
                ...
                # - "capped": Physics with limited external support (spring-damper on root XYZ)
                # - "grounded": Physics with XY + rotation support, but Z is free (robot stands on ground)
                # - "full": Kinematic replay (root pose set directly from motion)

                if harness_mode == "full":
                    # Kinematic replay - directly set root pose from GMR motion
                    ref_xy_delta = ref_root_pos[i, :2] - ref_xy_start
                    mj_data.qpos[0:2] = sim_xy_start + ref_xy_delta
                    mj_data.qpos[2] = ref_root_pos[i, 2] + z_offset
                    mj_data.qpos[3:7] = ref_root_rot[i]
                    mj_data.qvel[0:6] = 0
                    mj_data.qfrc_applied[:] = 0

                # Compute orientation (from current state, for metrics)
                ...
                stab_torque = np.zeros(3)

                if harness_mode == "capped":
                    # Full support with caps: XY + Z + rotation
                ...
                elif harness_mode == "grounded":
                    # Grounded mode: XY + rotation support, but Z is FREE (robot stands on ground)
                    ...
                    # Rotation stabilization (prevent flipping) - track reference orientation
                    # Higher gains to prevent tipping when contact is briefly lost
                    rot_kp = 200.0
                    rot_kd = 40.0
                    rot_cap = 20.0  # Cap torque to prevent instability

                    # Get reference pitch/roll
                    ref_w, ref_x, ref_y, ref_z = ref_root_rot[i]
                    ref_sinp = np.clip(2.0 * (ref_w * ref_y - ref_z * ref_x), -1.0, 1.0)
                    ref_pitch = np.arcsin(ref_sinp)
                    ref_sinr_cosp = 2.0 * (ref_w * ref_x + ref_y * ref_z)
                    ref_cosr_cosp = 1.0 - 2.0 * (ref_x * ref_x + ref_y * ref_y)
                    ref_roll = np.arctan2(ref_sinr_cosp, ref_cosr_cosp)

                    # PD control to track reference orientation
                    pitch_error = ref_pitch - curr_pitch
                    roll_error = ref_roll - curr_roll
                    stab_torque[0] = np.clip(rot_kp * roll_error - rot_kd * mj_data.qvel[3], -rot_cap, rot_cap)   # roll
                    stab_torque[1] = np.clip(rot_kp * pitch_error - rot_kd * mj_data.qvel[4], -rot_cap, rot_cap)  # pitch

                    # Z is FREE - no force applied, robot height determined by ground contact
                    stab_force_z = 0.0

                # Apply forces
                mj_data.qfrc_applied[0:2] = stab_force_xy
                mj_data.qfrc_applied[2] = stab_force_z
                mj_data.qfrc_applied[3:6] = stab_torque

                total_harness = np.sqrt(stab_force_z**2 + np.sum(stab_force_xy**2) + np.sum(stab_torque**2))
                ...
                if viewer is not None:
                    viewer.sync()
                    # Sync to real-time when rendering
                    if render:
                        time.sleep(ctrl_dt)

                # Check for catastrophic failure (skip in full mode - kinematic replay)
                if harness_mode != "full":
            ...
            # Exit loop if not in loop mode, or if fell and in loop mode (will restart)
            if not loop:
                break
            # In loop mode, continue to next iteration (will reset and restart)
...
def main():
    ...
    parser.add_argument(
        "--motion",
        type=str,
        default="training/data/gmr/walking_medium01.pkl",
    ...
    parser.add_argument(
        ...
        type=str,
        default="capped",
        choices=["none", "capped", "grounded", "full"],
        help="Harness mode: none, capped (default), grounded (XY+rot only, Z free), or full (kinematic)",
    )
    parser.add_argument(
        "--harness-cap",
        type=float,
    ...
    parser.add_argument(
        "--loop",
        action="store_true",
        help="Loop the simulation continuously",
    )
    parser.add_argument(
        "--z-offset",
        type=float,
        default=0.0,
        help="Z offset to apply to root position in meters (negative = lower robot)",
    )
