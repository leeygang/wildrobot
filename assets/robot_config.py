...
from mujoco_robot_config.json (generated by assets/post_process.py).
...
@dataclass
class RobotConfig:
    """Robot configuration loaded from mujoco_robot_config.json.
    ...
    # Internal: raw config for property access (not part of public API)
    ...
    def _get_root_sensor(self, key: str) -> str:
        ...
        if not root:
            raise ValueError("mujoco_robot_config.json missing 'root_spec' section")
        sensors = root.get("sensors")
        if not sensors:
            raise ValueError("mujoco_robot_config.json missing 'root_spec.sensors' section")
        if key not in sensors:
            raise ValueError(f"mujoco_robot_config.json missing 'root_spec.sensors.{key}'")
        return sensors[key]
    ...
    def _get_foot_spec(self, side: str, key: str) -> str:
        ...
        if not feet_config:
            raise ValueError("mujoco_robot_config.json missing 'foot_specs' section")
        foot = feet_config.get(side)
        if not foot:
            raise ValueError(f"mujoco_robot_config.json missing 'foot_specs.{side}' section")
        if key not in foot:
            raise ValueError(f"mujoco_robot_config.json missing 'foot_specs.{side}.{key}'")
        return foot[key]
    ...
    def contact_scale(self) -> float:
        ...
        if "contact_scale" not in feet_config:
            raise ValueError("mujoco_robot_config.json missing 'foot_specs.contact_scale'")
        return feet_config["contact_scale"]

    @classmethod
    def from_file(cls, config_path: str | Path) -> "RobotConfig":
        """Load robot configuration from JSON/YAML file.

        Args:
            config_path: Path to mujoco_robot_config.json
...
def load_robot_config(config_path: str | Path) -> RobotConfig:
    ...
    Args:
        config_path: Path to mujoco_robot_config.json (required).
    ...
    _robot_config = RobotConfig.from_file(config_path)
    return _robot_config
