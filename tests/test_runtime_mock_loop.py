...
class _SmallPolicy:
    ...
    def predict(self, obs):
        assert obs.shape == (937,), f"unexpected obs shape {obs.shape}"
...
def test_walk_policy_uses_native_17d_contract_and_direct_hardware_io(
    ...
    robot_io = MockRobotIO(
        actuator_names=policy_names,
        control_dt=runtime_policy_config.ctrl_dt,
        home_q_rad=policy_home,
    )
    ...
    assert v8_spec.model.action_dim == 17
    assert len(robot_io.actuator_names) == 17
    assert not any("wrist" in name for name in policy_names)
    assert info["obs"].shape == (937,)
    assert info["target_q_rad"].shape == (17,)
    assert robot_io.written[-1].shape == (17,)
