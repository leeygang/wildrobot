...
class HiwonderBoardActuators:
    ...
    def __init__(
        ...
        missing = [j for j in self.joint_names if j not in self.cfg.servo_ids]
        if missing:
    ...
    def disable(self) -> None:
        servo_ids = [self.cfg.servo_ids[j] for j in self.joint_names]
    ...
    def set_targets_rad(self, targets_rad: np.ndarray, move_duration_ms: Optional[int] = None) -> None:
        ...
        for joint_name, rad in zip(self.joint_names, targets, strict=True):
            offset = float(self.cfg.joint_offsets_rad.get(joint_name, 0.0))
            servo_id = int(self.cfg.servo_ids[joint_name])
    ...
    def get_positions_rad(self) -> Optional[np.ndarray]:
        ...
        for joint_name in self.joint_names:
            sid = self.cfg.servo_ids[joint_name]
            units = by_id.get(sid)
            if units is None:
                return None
            offset = float(self.cfg.joint_offsets_rad.get(joint_name, 0.0))
