...
**IMPORTANT**: Keep `training/CHANGELOG.md` synced with all AMP training changes!
...
- **Fail Fast, No Silent Defaults**: Configuration values must be explicitly provided. If a required config (like `mujoco_robot_config.json`) is missing, raise an error immediately rather than falling back to hardcoded defaults. This prevents subtle bugs from config mismatches.
